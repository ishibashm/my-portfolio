...
source: "https://azukiazusa.dev/blog/mcp-server-container-use/"
author:
  - "[[azukiazusaのテックブログ2]]"
published: 2025-07-13
created: 2025-09-23
description: "AI コーディングエージェントは便利ですが、任意の Bash コマンドを実行できるため、ユーザーのシステムに影響を与える可能性があります。Container Use は MCP サーバーとして動作し、AI コーディングエージェントにサンドボックス環境を提供します。この記事では Container Use の利用方法について紹介します。"
tags:
  - "clippings"
---
...
![](https://images.ctfassets.net/in6v9lxmm5c8/3AZVIynSvYKRYmwCFiimOt/5d455720ae18d4910d59bee2b8624b29/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2025-07-12_21.29.15.png?q=50&fm=webp)
...
![](https://images.ctfassets.net/in6v9lxmm5c8/1xcDq2j8I74xNAzU0d0Z0E/0f829538b0f10b0026bfe641bac0fccc/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2025-07-12_21.36.31.png?q=50&fm=webp)
...
![](https://images.ctfassets.net/in6v9lxmm5c8/3usyNcFErrjcfSQfgOCo2I/e084cf6bb1b4b290649baadaa1ba4b74/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2025-07-12_21.43.57.png?q=50&fm=webp)
...
![](https://images.ctfassets.net/in6v9lxmm5c8/9O8B0AgSDxT3RlODfwwtx/fca96a8301960fbfca2cbb5025828369/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2025-07-12_21.56.50.png?q=50&fm=webp)

コンテナ内でポート `5173` で開発サーバーが起動されたうえで、 [http://127.0.0.1:51121](http://127.0.0.1:51121/) を通じてアクセスできるように設定されます。実際にアクセスしてみると TODO アプリケーションが表示されます（今回はスタイリングに失敗しているようですが...）。

![](https://images.ctfassets.net/in6v9lxmm5c8/5dyMMMwSIM4F4x8aQCrv6z/a8aa97273bf9aae84c98a974b06b50bd/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2025-07-12_22.00.56.png?q=50&fm=webp)
...
## Container Use のユースケース

Container Use は、AI コーディングエージェントの安全な利用を促進するだけでなく、さまざまなユースケースで活用できます。

### 安全なコマンド実行環境

AI コーディングエージェントに `rm -rf /` のような危険なコマンドを実行させないためのサンドボックス環境として利用できます。これにより、ローカル環境を破壊するリスクを回避できます。

### プロジェクトごとの独立した開発環境

複数のプロジェクトを並行して進めている場合でも、Container Use を使えばプロジェクトごとに独立した開発環境を簡単に構築できます。これにより、依存関係のコンフリクトを気にすることなく、クリーンな環境で開発を進めることができます。

### CI/CD パイプラインへの統合

Container Use は、CI/CD パイプラインに統合することも可能です。例えば、GitHub Actions のワークフロー内で Container Use を利用して、安全な環境でテストやビルドを実行できます。

### 教育・トレーニング目的での利用

プログラミングの学習者が安全にコードを実行できる環境を提供するためにも利用できます。初心者が誤ってシステムに影響を与えることなく、自由にコマンドを試せる場として活用できます。

## おわりに

この記事では、AI コーディングエージェントにサンドボックス環境を提供する MCP サーバーである Container Use について紹介しました。Container Use を活用することで、AI の強力な支援を受けながらも、開発環境の安全性を確保できます。

AI コーディングエージェントの利用が広がる中で、その利便性とリスクを正しく理解し、適切に付き合っていくことが重要です。Container Use は、そのための強力なツールとなるでしょう。
...
### サンドボックス環境内でコマンドを実行する MCP ツールはどれですか？

- environment_run_cmd
  正解！
  environment_run_cmd はサンドボックス環境内でコマンドを実行するための MCP ツールです。
- env_run_cmd
  もう一度考えてみましょう
- environment_exec_cmd
  もう一度考えてみましょう
- run_cmd
  もう一度考えてみましょう

### AI コーディングエージェントの実行ログを確認するコマンドはどれですか？

- container-use log <env_id>
  正解！
  container-use log <env_id> コマンドで、サンドボックス環境内で実行されたコマンドのログを確認できます。
- container-use checkout <env_id>
  もう一度考えてみましょう
  container-use checkout <env_id> は git ブランチに切り替えて作業内容を確認するためのコマンドです。
- container-use diff <env_id>
  もう一度考えてみましょう
  container-use diff <env_id> はファイルの差分を確認するためのコマンドです。
- container-use terminal <env_id>
  もう一度考えてみましょう
  container-use terminal <env_id> はサンドボックス環境内のシェルに接続するためのコマンドです。
